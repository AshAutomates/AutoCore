...
def ocr(x=None, y=None, width=None, height=None):
    """
    Extract text from screen using OCR (Optical Character Recognition).
    Extracts English text only. Returns text in lowercase.

    Args:
        x, y: Top-left corner coordinates of region (optional)
        width, height: Dimensions of the region (optional)

    Returns:
        str: Extracted text from screen in lowercase, or None if error

    Examples:
        # Read entire screen
        text = ocr()

        # Read specific region (x, y, width, height)
        text = ocr(100, 200, 400, 400)
    ...
    Note:
        First run downloads OCR models (~100MB), subsequent runs are fast.
    """
    try:
        # Get shared OCR reader
        reader = _get_ocr_reader()
        ...
        if not results:
            print(f"OCR: No text found in the specified region.")
            return ""  # returning blank string and not None so as no error will be thrown in blank text case
